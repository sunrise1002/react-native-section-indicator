...
const SectionTitle: React.FC<SectionTitleProps> = memo(({ sectionTitle, y, index }) => {
  const sectionTitleAnimatedStyle = useAnimatedStyle(() => {
    const startYCoordinate = 0 + PADDING_VERTICAL + index * SECTION_TITLE_HEIGHT
    const isHighlight = startYCoordinate <= y.value && y.value <= startYCoordinate + SECTION_TITLE_HEIGHT
  ...
  const sectionAnimatedStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      backgroundColor: isHighlight ? Colors.white : Colors.profileBackground,
  ...
  return (
    <Animated.View style={[sectionAnimatedStyle]}>
      <Animated.Text style={[styles.section, sectionTitleAnimatedStyle]}>{sectionTitle}</Animated.Text>
    </Animated.View>
  )
})

const SectionListScrollIndicator: React.FC<SectionListScrollIndicatorProps> = ({ sectionTitles, sectionListRef }) => {
        ...
        () =>
          sectionListRef?.current?.scrollToLocation({
            ...
            sectionIndex: Math.min(
              parseInt(`${(currentYCoordinate - PADDING_VERTICAL) / SECTION_TITLE_HEIGHT}`, 10),
  ...
  const renderSectionTitles = sectionTitles.map((sectionTitle: string, index: number) => (
    <SectionTitle sectionTitle={sectionTitle} index={index} y={y} />
  ))

  return (
    <PanGestureHandler onGestureEvent={eventHandler}>
      <Animated.View style={styles.container}>{renderSectionTitles}</Animated.View>
    </PanGestureHandler>
  )
}
